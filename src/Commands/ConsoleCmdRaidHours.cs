                ...
                if (_params.Count != 0)
                {
                    switch (_params[0].ToLower())
                    {
                        ...
                        case "fix":
                            ...
                            BagDropManager.RefreshBagDropOnLogoutState(player, player.GetBlockPosition());
                            SdtdConsole.Instance.Output($"Current raid and bag-drop states have been re-sent to player {player.GetDebugName()}.");
                            return;
                        case "settings":
                            SdtdConsole.Instance.Output(SettingsManager.AsString());
                            return;
                        case "gt":
                            SdtdConsole.Instance.Output($"Current, real-world server time: {TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, SettingsManager.TimeZoneInfo)} ({SettingsManager.TimeZoneInfo.DisplayName})");
                            return;
                        case "timezones":
